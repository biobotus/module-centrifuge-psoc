...
void CAN_1_ReceiveMsg(uint8 rxMailbox) 
{
    ...
    *******************************************************************************/
    void CAN_1_ReceiveMsg0(void) 
    {
        ...
        // Loading the data length and the ID of the incomming message.
        receivedMsgs[msgToBeStoredIndex].DLC = CAN_1_GET_DLC(0);
        receivedMsgs[msgToBeStoredIndex].CAN_ID = CAN_1_GET_RX_ID(0);
        
        int i = 0;
        
        // Get pay load and save it to the receivedMessages table.
        for (i=0; i<receivedMsgs[msgToBeStoredIndex].DLC; i++)
        {
            receivedMsgs[msgToBeStoredIndex].msgBytes.byte[i] = (uint8)CAN_1_RX_DATA_BYTE(0,i); // Write the received char in the message received buffer.            
        }
        
        // If the message requires an immediate action, we do it right away.
        // TODO : Make a switch case on all the instructions that require an immediate action -> actions that influence blocking function as calibration & temperature maintain.
        /*if (receivedMsgs[msgToBeStoredIndex].msgBytes.byte[INDEX_INSTRUCTION_BYTE] == STOP_CALIBRATION)
        {
            STOP_CALIBRATION_FLAG = 1;            
            
            // Echo back to the master right away.
            CAN_MSG toSend;
            toSend.CAN_ID = MASTER_ID; // No need to define this time, as we echo back to the master with the dedicated function.
            toSend.msgBytes = receivedMsgs[msgToBeStoredIndex].msgBytes;
            toSend.DLC = receivedMsgs[msgToBeStoredIndex].DLC;            
            echoBackToMaster(&toSend);
        }
        else
        {                   
            // Increment the amount of messages to be treated in the main loop.
            messageToBeTreatedCount++; 
        } 
        */
        messageToBeTreatedCount++; 
        // If we reached the limit of the message buffer table, we set the next one to 0, otherwise we increment it by one.
        if (msgToBeStoredIndex == MAX_MSG_BUFFER_SIZE)
        {
            msgToBeStoredIndex = 0;
        }
        else
        {
            msgToBeStoredIndex++; // Increment the counter indicating at which index starts the last message.
        }
        /* `#END` */
